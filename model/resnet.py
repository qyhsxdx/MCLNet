...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, dilation=1, adapters = False):
        ...
        if self.adapters:
            self.adapt = nn.Conv2d(planes, planes, kernel_size=1, stride=stride, bias=False)
    ...
    def forward(self, x):
        ...
        out = self.relu(out)
        if self.adapters:
          out = self.adapt(out) + self.conv2(out)
        else:
          out = self.conv2(out)
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, last_conv_stride=2, last_conv_dilation=1):
        ...
        self.layer1 = self._make_layer(block, 64, layers[0], adapters=False)
        self.layer2 = self._make_layer(block, 128, layers[1], stride=2, adapters=False)
        self.layer3 = self._make_layer(block, 256, layers[2], stride=2, adapters=False)
        self.layer4 = self._make_layer(block, 512, layers[3], stride=last_conv_stride, dilation=last_conv_dilation, adapters=False)
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilation=1, adapters = False):
        downsample = None
        ...
        layers = []
        layers.append(block(self.inplanes, planes, stride, downsample, dilation, adapters))
